...
const ManualEntry = ({ onAnalyze, onCancel }: ManualEntryProps) => {
    ...
    const runningStats = useMemo(() => {
        const values = rows
            .map(r => parseFloat(r[outcomeName]))
            .filter(v => !isNaN(v));
    ...
    const handleKeyDown = (e: React.KeyboardEvent, rowIdx: number, colIdx: number, columns: string[]) => {
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-slate-200 p-8">
                <div className="w-full max-w-lg bg-slate-800 rounded-xl border border-slate-700 p-8 shadow-2xl">
                    ...
                    <div className="mb-8">
                        <label className="block text-sm font-semibold text-slate-400 mb-2">Spec Limits (optional)</label>
                        <p className="text-xs text-slate-500 mb-3">Set limits to see pass/fail feedback as you enter data</p>
        ...
        <div className="flex flex-col h-screen bg-slate-900 text-slate-200">
            ...
            <div className="flex-none p-4 border-b border-slate-700 bg-slate-800/50">
                <div className="flex justify-between items-center mb-3">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setStep('setup')} className="bg-slate-800 p-3 rounded-lg hover:bg-slate-700 border border-slate-600">
                    ...
                    <div className="flex gap-3">
                        <button onClick={handlePaste} className="px-4 py-3 rounded-lg border border-slate-600 text-slate-300 text-sm hover:bg-slate-700 flex items-center gap-2">
                            <Clipboard size={18} /> Paste
                        </button>
                        <button onClick={onCancel} className="px-4 py-3 rounded-lg text-slate-400 hover:text-white text-sm">
                            Cancel
                        </button>
                        <button
                ...
                {runningStats && (
                    <div className="flex gap-6 bg-slate-900/50 rounded-lg p-3 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono font-bold text-blue-400">{runningStats.mean.toFixed(2)}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-slate-500">Range:</span>
                            <span className="font-mono text-slate-300">{runningStats.min.toFixed(1)} – {runningStats.max.toFixed(1)}</span>
                        </div>
                        {(lsl || usl) && (
                            <div className="flex items-center gap-2 ml-auto">
                                <span className="text-slate-500">Pass:</span>
                                <span className={`font-mono font-bold ${runningStats.passCount === runningStats.count ? 'text-green-400' : 'text-amber-400'}`}>
            ...
            <div className="flex-1 overflow-auto p-8">
                            ...
                            {rows.map((row, idx) => {
                                ...
                                return (
                                    <tr key={idx} className="border-b border-slate-700/50 hover:bg-slate-700/30 group">
                                        <td className="p-3 text-center text-slate-600 font-mono text-sm min-h-[56px]">{idx + 1}</td>
                ...
                <div className="max-w-5xl mx-auto mt-4 text-center text-slate-500 text-sm">
                    <kbd className="bg-slate-800 px-2 py-1 rounded border border-slate-600 font-mono text-xs">Enter</kbd> or
                    <kbd className="bg-slate-800 px-2 py-1 rounded border border-slate-600 font-mono text-xs ml-1">Tab</kbd> to move between cells.
                    New rows are added automatically at the end.
                </div>
            </div>
        </div>
    );
