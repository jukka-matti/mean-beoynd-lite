...
export function detectColumns(data: any[]) {
    ...
    const catCols = keys.filter(k => typeof data[0][k] === 'string' || typeof data[0][k] === 'boolean');

    // Heuristic: First numeric col is outcome, first 3 strings are factors
    return {
        outcome: numericCols[0] || null,
        factors: catCols.slice(0, 3),
        timeColumn: keys.find(k => k.toLowerCase().includes('time') || k.toLowerCase().includes('date')) || null
    };
}
